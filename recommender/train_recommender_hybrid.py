...
class RecommenderDataset(Dataset):
    ...
    def _load_feature_extractor_models(self):
        self.image_feat_extractor = torchvision.models.resnet50(pretrained=True)

        # Remove the last fc layer
        self.image_feat_extractor.fc = nn.Identity()
    ...
    def _load_image_data(self):
        self.image_transform = transforms.Compose([
            transforms.Resize(size=(args.input_image_size, args.input_image_size)),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ])

        image_paths_list = glob.glob(os.path.join(args.image_data_dir, "*", "*.png"))

        for idx, cur_image_path in enumerate(tqdm(image_paths_list, desc="LOG: Loading Image data")):
            cur_image = Image.open(cur_image_path).convert('RGB')
            cur_image_tensor = self.image_transform(cur_image).unsqueeze(0)
            cur_image_features = self.image_feat_extractor(cur_image_tensor).squeeze().detach().cpu()
            self.image_feature_map[cur_image_path] = cur_image_features

            # free up memory
            del cur_image
            del cur_image_tensor
    ...
    def _getImageFeatures(self, input_image_path):
        image_features = self.image_feature_map[input_image_path]
    ...
    def _build(self):
        ...
        for idx, cur_row in tqdm(df_data.iterrows(), total=df_data.shape[0], desc="LOG: Loading Text data"):
            try:
                ...
                cur_image_features = self._getImageFeatures(cur_image_path).squeeze()
                cur_image_meta_features = self._getImageMetaFeatures(cur_image_path, cur_image_meta_text).squeeze()
                cur_inputs = torch.cat((cur_text_features, cur_image_features, cur_image_meta_features))
...
if __name__ == "__main__":
    ...
    recommender = RecommenderModel(input_size=3584, num_classes=2, hidden_size=500)
