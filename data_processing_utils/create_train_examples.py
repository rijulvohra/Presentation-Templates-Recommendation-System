...
def main():
    ...
    train_text_filename = 'data/topic_classification_data/train.csv'
    train_examples_filename = 'data/recommender_data/train.csv'
    ...
    out_cols = ['text', 'topic', 'image_label', 'match']
    out_data = []
    with open(train_text_filename, mode='r', encoding='utf-8') as train_text_file, \
        ...
        for text_line in csv_reader:
            ...
            if topic not in topic_pngs_dict:
                ...
                out_data.append([text_line[0], topic, postive_png_name, 1])
                ...
                out_data.append([text_line[0], negative_topic, negative_png_name, 0])


    df = pd.DataFrame(data=out_data, columns=out_cols)
    df.to_csv(train_examples_filename, index=False)
